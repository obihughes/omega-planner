...
export function useProjects() {
  ...
  const reorderProjects = useCallback((activeId: string, overId: string) => {
    ...
    updateProjectsState(() => updatedProjectsWithOrder);
  }, [projects]);
    ...
    updateProjectsState(prevProjects => 
      prevProjects.map(p => {
        if (p.id === projectId) {
          ...
          return { ...p, tasks: updatedTasks, progress, updatedAt: new Date().toISOString() };
        }
        return p;
      })
    );
  ...
  const reorderTasksInProject = useCallback((projectId: string, activeId: string, overId: string) => {
    ...
    updateProjectsState(prevProjects => {
      ...
      project.tasks = reorderedTasks.map((task, index) => ({ ...task, order: index, updatedAt: new Date().toISOString() }));
