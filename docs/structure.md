...
- `components/goal-hierarchy` – Goal Hierarchy (month → week → day planning; month/week use summary + sub-goals, days store plain text in `summary` with `--` bullet shortcuts via `DayGoalTextarea`)
