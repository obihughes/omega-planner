...
export interface SchedulingSidebarProps {
  poolTasks: Task[];
  ...
  onApplySavedDay: (savedDayId: string, replace: boolean) => void;
}
...
export function SchedulingSidebar({
  ...
  const getTasksForDate = (date: Date) => {
    ...
    return [...scheduled, ...pool, ...pinned].filter(
  ...
  const handleDragStart = (e: React.DragEvent, task: Task, isFromPool: boolean = false) => {
    ...
    e.dataTransfer.setData(
      'text/plain',
      JSON.stringify({ task, isFromPool: taskIsFromPool, type: 'task-assignment' })
    );
  ...
  const handleDrop = (e: React.DragEvent, targetDate: Date | null = null) => {
    ...
    try {
      ...
      if (data.type === 'task-assignment' && data.task) {
        ...
        } else if (!isFromPool && task.startHour !== undefined) {
          onUnassignTask(task);
        }
      }
    } catch (error) {
  ...
  return (
    <div className="w-72 shrink-0 border-r border-border bg-card/30 flex flex-col h-full">
          ...
          <div className="flex items-center gap-1">
            ...
            <Popover open={bulkActionsOpen} onOpenChange={setBulkActionsOpen}>
              <PopoverTrigger asChild>
                <Button variant="ghost" size="sm" className="h-6 w-6 p-0 hover:bg-accent" title="Bulk actions">
                ...
                <div className="space-y-1">
                  ...
                  {savedDays.length > 0 && (
                    <div className="pt-2 border-t border-border">
                      <p className="text-xs font-medium text-muted-foreground px-2 pb-1">Apply saved day</p>
                      <div className="max-h-40 overflow-y-auto space-y-1">
                        {savedDays.map((savedDay) => (
                          <div key={savedDay.id} className="flex items-center gap-1 px-1">
                            <span className="text-xs truncate flex-1" title={savedDay.name}>
                              {savedDay.name}
                            </span>
                            <Button
                              size="sm"
                              ...
                              onClick={() => {
                                onApplySavedDay(savedDay.id, false);
                                setBulkActionsOpen(false);
                              }}
                            >
                              Apply
                            ...
                            <Button
                              ...
                              className="h-6 px-2 text-[10px]"
                              onClick={() => {
                                onApplySavedDay(savedDay.id, true);
                                setBulkActionsOpen(false);
                              }}
                            >
                              Replace
              ...
              <div
                ...
                title={
                  tasksForDay.length > 0
                    ? `${tasksForDay.length} task${tasksForDay.length !== 1 ? 's' : ''} (${scheduledCount} scheduled, ${unscheduledCount} unscheduled)`
                    : ''
                }
              >
                ...
                {tasksForDay.length > 0 && (
                  <div className="absolute bottom-0.5 left-1/2 transform -translate-x-1/2 flex gap-0.5">
                    {scheduledCount > 0 && <div className="w-1 h-1 bg-blue-500 rounded-full" />}
                    {unscheduledCount > 0 && <div className="w-1 h-1 bg-orange-500 rounded-full" />}
                    {tasksForDay.length > 2 && <div className="w-1 h-1 bg-muted-foreground rounded-full" />}
                  </div>
                )}
              </div>
            );
      ...
      <div className="flex-1 flex flex-col min-h-0">
        ...
        <div
          className={cn(
            'flex-1 p-4 space-y-3 overflow-y-auto transition-all duration-200 min-h-0',
